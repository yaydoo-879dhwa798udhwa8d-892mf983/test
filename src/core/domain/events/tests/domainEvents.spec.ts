...
import { MockRegisterJob } from "./mocks/services/mockRegisterJob";
import { MockJobAggregateRootId } from "./mocks/domain/mockJobAggregateRootId";
import { UniqueEntityID } from "../../UniqueEntityID";

let board: MockRegisterJob;
...
describe("Domain Events", () => {
    ...
    describe("Given a JobCreatedEvent, JobDeletedEvent and a RegisterJob handler class", () => {
        it("Should be able to setup event subscriptions", () => {
            board = new MockRegisterJob();
            board.setupSubscriptions();

            expect(Object.keys(DomainEvents["handlersMap"]).length).toBe(2);

            expect(
                DomainEvents["handlersMap"][MockJobCreatedEvent.name].length
            ).toBe(1);
            expect(
                DomainEvents["handlersMap"][MockJobDeletedEvent.name].length
            ).toBe(1);
        });
        ...
        it("Should add the event to the DomainEvents list when the event is created", () => {
            ...
            var domainEventsAggregateSpy = sinon.spy(
                DomainEvents,
        ...
        it("Should call the handlers when the event is dispatched after marking the aggregate root", () => {
            ...
            var jobCreatedEventSpy = sinon.spy(board, "handleJobCreatedEvent");
            var jobDeletedEventSpy = sinon.spy(board, "handleDeletedEvent");
        ...
        it("Should only add the domain event to the ", () => {
            ...
            expect(
                DomainEvents["markedAggregates"][1].domainEvents.length
            ).toBe(2);

            // And the first aggregate should have one event
            expect(
                DomainEvents["markedAggregates"][0].domainEvents.length
            ).toBe(1);
